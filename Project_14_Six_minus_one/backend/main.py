...
def is_accessibility_scope_message(message: str) -> bool:
    ...
    if not text:
        return True
    # Allow broad product/page-improvement questions by default.
    allow_keywords = [
        "website",
        "web",
        "page",
    ...
    # Block only clearly unrelated intents.
    deny_keywords = [
        "weather",
        "temperature",
        "forecast",
    ...
    # Simple math / factual trivia style prompts are out-of-scope.
    if "=" in text and any(ch.isdigit() for ch in text):
        return False

    # Default allow to avoid false rejections.
    return True
